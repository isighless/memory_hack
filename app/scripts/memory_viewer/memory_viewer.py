...
class MemoryControl(controls.Group):
    ...
    def generate(self):
        ...
        if self.memory_buffer:
            ...
            for col_index, header in enumerate(self.column_headers):
                ...
                self.html_data += '<ons-col class="col ons-col-inner" style="{}">{}</ons-col>\n'.format(style, header)
        ...
        for row_index in range(0, self.max_rows):
            ...
            for col_index in range(0, self.max_cols):
                ...
                if index < len(self.memory_buffer):
                    ...
                    self.html_data += '<ons-col class="col ons-col-inner" style="{}">'.format(style)
                    self.html_data += (
                        '<button id="{}_membutton-{:03}" class="memory_button" '
                        'style="background:transparent; border:none; width:100%;" '
                        'data-mv_address="{}" '
                        'onclick="script.script_interact_button(event)">{:02X}</button>'
                    ).format(
                        self.script_ids[-1],
                        index + self.start_address,
                        self.start_address + index,
    ...
    def _column_style(self, col_index: int, header: bool = False) -> str:
        if header:
            background = '#f5f7fb' if (col_index % 2) == 0 else '#e8ecf5' # slightly darker header background
            font_weight = 'font-weight: 600;'
        else:
            background = '#ffffff' if (col_index % 2) == 0 else '#f5f7fb'
            font_weight = 'font-size: smaller;'
        padding = 'padding: 4px 0;'
        text_align = 'text-align: center;'
        return f'background-color: {background}; {font_weight} {padding} {text_align}'
    ...
    def set_data(self, data:bytes, address: int = 0, base_address: int | None = None):
        ...
        if self.start_address <= self.current_selected_address < self.start_address + len(data):
            self.js(
                'if ($(".memory_button").css("border").indexOf("none") >= 0) {{ '
                '$("[data-mv_address=\'{}\']").css("border","solid"); '
                '$("[data-mv_address=\'{}\']").css("border-width", "1px"); '
                '}}'.format(self.current_selected_address, self.current_selected_address)
            )
